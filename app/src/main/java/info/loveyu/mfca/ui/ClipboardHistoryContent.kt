...
fun ClipboardHistoryContent(
    ...
    fun loadRecords() {
        scope.launch(Dispatchers.IO) {
            val result = dbHelper.query(
                keyword = searchKeyword.ifBlank { null }, limit = 200
            )
    ...
    if (showTimeCleanDialog) {
        TimeCleanDialog(
            dbHelper = dbHelper,
            onDismiss = { showTimeCleanDialog = false },
            onDeleted = { deleted ->
                Toast.makeText(context, "已清理 $deleted 条记录", Toast.LENGTH_SHORT).show()
                loadRecords()
            }
        )
    }
    if (showPasswordCleanDialog) {
        PasswordCleanDialog(
            dbHelper = dbHelper,
            onDismiss = { showPasswordCleanDialog = false },
            onDeleted = { deleted ->
                Toast.makeText(context, "已清理 $deleted 条记录", Toast.LENGTH_SHORT).show()
                loadRecords()
            }
    ...
    if (showVerificationCodeCleanDialog) {
        VerificationCodeCleanDialog(
            dbHelper = dbHelper,
            onDismiss = { showVerificationCodeCleanDialog = false },
            onDeleted = { deleted ->
                Toast.makeText(context, "已清理 $deleted 条记录", Toast.LENGTH_SHORT).show()
                loadRecords()
            }
