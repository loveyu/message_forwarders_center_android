...
object M2mManager {
    ...
    fun fetchProviders(): Result<Map<String, M2mProviderInfo>> {
        val port = apiPort
        if (port == 0) return Result.failure(IllegalStateException("API not ready"))
        return try {
            val providers = mutableMapOf<String, M2mProviderInfo>()

            // Fetch proxy providers
            val proxyUrl = URL("http://127.0.0.1:$port/providers/proxies")
            ...
            val ruleUrl = URL("http://127.0.0.1:$port/providers/rules")
    ...
    fun updateProvider(name: String, type: M2mProviderType): Result<Unit> {
        ...
        return try {
            ...
            val url = URL("http://127.0.0.1:$port/providers/$endpoint/$name")
            val conn = url.openConnection() as HttpURLConnection
    ...
    fun updateAllProviders(type: M2mProviderType): Result<Unit> {
        ...
        return try {
            ...
            val url = URL("http://127.0.0.1:$port/providers/$endpoint")
            val conn = url.openConnection() as HttpURLConnection
            conn.requestMethod = "PUT"
            conn.doOutput = true
