...
import info.loveyu.mfca.m2m.config.M2mProviderClient
import info.loveyu.mfca.m2m.models.M2mCandidateState
import info.loveyu.mfca.m2m.models.M2mConfigCacheState
...
import info.loveyu.mfca.m2m.traffic.M2mTrafficStatsCollector
...
object M2mManager {
    ...
    private val trafficCollector = M2mTrafficStatsCollector()

    val trafficStats: StateFlow<M2mTrafficStats?> get() = trafficCollector.trafficStats
    ...
    fun onTunEstablished(ifaceName: String?) {
        trafficCollector.onTunEstablished(ifaceName)
    }

    fun onApiReady(port: Int, secret: String, scope: CoroutineScope) {
        apiPort = port
        apiSecret = secret
        trafficCollector.apiPort = port
        trafficCollector.apiSecret = secret
        trafficCollector.onApiReady(port, secret, scope)
    }

    fun onTunDestroyed() {
        trafficCollector.onTunDestroyed()
        apiPort = 0
        apiSecret = ""
    }

    fun stopTrafficPolling() {
        trafficCollector.stopTrafficPolling()
    }

    fun refreshTrafficStats() {
        trafficCollector.refreshTrafficStats()
    }

    fun logTrafficStats() {
        trafficCollector.logTrafficStats()
    }
    ...
    // ── Provider (external resource) API via m2m core REST API ──

    fun isApiReady(): Boolean = apiPort > 0

    fun getApiPort(): Int = apiPort

    fun getApiSecret(): String = apiSecret

    fun fetchProviders(): Result<Map<String, M2mProviderInfo>> =
        M2mProviderClient.fetchProviders(apiPort, apiSecret)

    fun updateProvider(name: String, type: M2mProviderType): Result<Unit> =
        M2mProviderClient.updateProvider(apiPort, apiSecret, name, type)

    fun updateAllProviders(type: M2mProviderType): Result<Unit> =
        M2mProviderClient.updateAllProviders(apiPort, apiSecret, type)
}
