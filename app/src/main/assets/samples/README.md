...
| `20_vpn_input.yaml` | VPN 输入 - 远程 mihomo 配置、核心地址、候选切换、应用过滤 |
...
```
注意：JSON 外层 `{` 必须用 `{{` 转义，否则会被模板引擎当作表达式起始符消费。等同于一步 `$data` 替换。
...
| `jsonEncode(data)` | JSON 字符串编码（返回带引号的 JSON 字符串值） | `"he said \"hello\""` |
...
在 `transform.call` 中按顺序调用外部服务，将结果存为命名变量供后续步骤使用：
...
call:                                        # 顶层资源定义
  - name: resource-upload
    ...
    method: POST
    headers:
      content-type: "{args[1].content-type}" # 支持 {args[N].field} 取调用参数字段
    body: "{args[0]}"                        # 支持 {args[N]} 取第 N 个调用参数
    response: "{response}"                   # 支持 {response} 取响应体
    timeout: 15s
    retry:
      maxAttempts: 3
      interval: 1s
...
rules:
  - name: example
    ...
    pipeline:
      - transform:
          call:
            - uploaded: "resource-upload(data, headers)" # 调用资源，结果存为 uploaded
            - aiResult: "ai-analyze(uploaded, data)"     # 可使用前一步变量作为参数
          format: "{aiResult.answer}"                    # 格式化中可引用 call 变量
        to: [output]
```
...
| 参数名 | 说明 |
|--------|------|
| `data` | 当前 pipeline 数据（字符串） |
| `headers` | 当前 pipeline headers（Map，序列化为 JSON 传入） |
| `varName` | 同一步骤中前一个 call 产生的变量 |

**模板变量**（在 url / headers / body / response 中可用）

| 变量 | 说明 |
|------|------|
| `{args[N]}` | 第 N 个调用参数（整体；Map 被序列化为 JSON 字符串） |
| `{args[N].field}` | 第 N 个调用参数中的字段（Map 或 JSON 对象） |
| `{response}` | HTTP 响应体原始字符串 |
| `{$responseCode}` | HTTP 响应状态码 |

**后续步骤中的 call 变量**（format / filter 中可用）

| 变量 | 说明 |
|------|------|
| `{varName}` | call 步骤结果整体（非 JSON 对象时为字符串） |
| `{varName.field}` | call 步骤结果中的字段（结果为 JSON 对象时） |

**结果覆盖规则**

若调用返回值是包含 `data` 键的 JSON 对象 → 覆盖当前 pipeline 的 `data`  
若调用返回值是包含 `headers` 键的 JSON 对象 → 覆盖当前 pipeline 的 `headers`

详见 `19_call_resource.yaml` 完整示例。
